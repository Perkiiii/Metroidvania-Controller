...
- **All audio goes through `AudioManager.PlaySFX` or `PlayMusic`.** Action classes and modules pass an `AudioClip` field to `AudioManager.Instance.PlaySFX`. Direct `AudioSource.Play()` calls on hero or enemy prefabs are not permitted. `Time.timeScale` changes are owned exclusively by `GameManager.Pause()` / `Unpause()` — nothing else sets it.
