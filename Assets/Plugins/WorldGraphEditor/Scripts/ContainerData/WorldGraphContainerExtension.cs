...
namespace WorldGraphEditor
{
    public sealed partial class WorldGraphContainer
    {
        ...
        [Obsolete("Use EditorGraph Instead")]
        public ContainerEditorData EditorData => _editorData;

        public EditorGraph EditorGraph => _editorData?.EditorGraph;

        public bool HasData => _editorData != null && GetScenesData() != null && GetEdgesData() != null;

        private void SetEditorData(ContainerEditorData newEditorData)
        {
            if (_editorData == newEditorData)
                return;

            _editorData?.DisposeGraph();
            _editorData = newEditorData;
        }
