...
2. **Motion** (coroutine, started alongside fade-in after destination camera readiness): per-gate scripted movement until grounded or timeout, with `HeroMotor.EndScriptedEntry` + `RemoveControlLock` guaranteed in a `finally` block.
...
3. **`BeginSceneTransition(...)`** — public compatibility facade. It accepts either the legacy `(targetScene, destinationPassageGuid)` pair or a typed `SceneTransitionRequest`, then delegates to `SceneTransitionManager`. `SceneTransitionManager` validates the scene through `SceneLoader`, rejects duplicate requests, resolves the fade profile from request override → kind default → `CameraFade` fallback, records an ordered debug trace, and owns the normal transition coroutine. Actual flow: acquire the transition-owned hard camera freeze → add the outgoing hero control lock and grant i-frames → fade fully out → `SceneLoader.LoadSingle` → `OnSceneLoaded` fires `SceneInit`, then caches the new hero and resolves saved respawn placement → if a pending normal-death respawn exists, place at the checkpoint marker and skip gate entry → otherwise resolve destination `TransitionPoint` by WGE passage GUID → place hero at the destination gate or deterministic missing-gate fallback (`RespawnMarker`, then authored position; placement writes the hero `Transform` immediately so same-frame readiness frames the true gate, not the stale pre-teleport `Rigidbody2D.position`) → refresh `_sceneFallbackPosition` → await `GameCameras` destination readiness while black → begin fade-in (camera still frozen at the framed gate) → once the fade has started to reveal, **release the transition freeze and then begin** per-gate entry motion, so the camera follows the walk-in live → wait for both → restore `GameState.Playing`. The transition freeze is released at that reveal seam (or in guaranteed `finally` cleanup on the no-entry/failure paths); its release suppresses the `OverrideReleased` blend so no camera correction occurs after the walk-in. `GameCameras` owns readiness, immediate target/rendered-camera positioning, overlap refresh, smoothing reset, timeout, and direct-snap fallback. `HeroSceneEntry` owns entry motion, and `HeroMotor` remains the sole normal hero velocity authority.
...
`OverrideReleased` correction after reveal. `HeroSceneEntry` still owns directional entry motion,
and `HeroMotor` still owns every scripted-entry velocity write.
