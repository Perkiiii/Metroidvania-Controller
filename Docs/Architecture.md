...
**`PersistenceLifetime` enum** (`Scripts/World/Persistence/Core/PersistenceLifetime.cs`: `RoomRuntime` | `UntilDeath` | `Permanent`) is shared by `PersistentSwitch` and `PersistentBreakable`. `PersistentDoor` has no lifetime field — per the plan's policy table a door/shortcut is always a permanent physical fact, so it always calls `TryGetObjectState`/`SetObjectState` directly.
...
All three participants follow the same initialization-order contract as `EnemyPersistence`/`AbilityPickup`: query the registry exactly once in `Awake`, apply the result quietly (`playFeedback: false`), configure collider/visual/interactable state before any gameplay input can reach them, and only ever write the registry from a confirmed player action (`Interact()` / `ReceiveHeroAttack`), never from restoration.

**Room visitation.** `GameManager.OnSceneLoaded` calls `worldStateRegistry.MarkRoomVisited(scene.name)` directly — scene name is the room ID for this milestone (the project's three gameplay scenes are each a distinct room; no separate room-identity component exists, and none is needed while a scene *is* a room). This adds no scene-wide search and no polling: it rides the same `SceneManager.sceneLoaded` subscription `GameManager` already uses for hero placement and respawn-marker resolution, so it fires exactly once per confirmed scene load and never for the Boot scene itself (`GameManager` subscribes only after Boot has already finished loading).
